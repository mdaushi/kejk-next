...
const Nav = () => {
  ...
  return (
    <div className={`${sans.variable}`}>
      {/* Desktop nav */}
      <NavigationMenu.Root className="hidden backdrop-blur-md dark:border-zinc-700 md:fixed md:top-0 md:z-50 md:mx-auto md:flex md:h-16 md:w-full md:items-center md:justify-center md:rounded-none md:border md:border-none md:border-zinc-200 md:bg-white md:px-4 md:py-0 md:shadow-none md:duration-500 dark:md:bg-black/50 dark:md:shadow-none lg:px-0">
        <div className="mx-auto w-full justify-center lg:px-8">
          <div className="mx-auto flex max-w-7xl items-center justify-between">
            <div className="flex h-full w-full items-center">
              ...
              <p className="w-max px-2 pb-0 text-sm text-zinc-500 dark:text-zinc-400">
                {"Press"}
              </p>
              <code
                className={`${mono.variable} w-max font-mono text-sm text-lime-700 dark:text-lime-500`}
              >
                {`${key} + K`}
              </code>
              <p className="w-max px-2 pb-0 text-sm text-zinc-500 dark:text-zinc-400">
                    ...
                    {item.hasMultiple && (
                      <>
                        <span className="nav">
                          {item.label}
                          <ChevronDownIcon
                            className="ml-2 h-4 w-4 flex-shrink-0 text-zinc-500 transition-transform duration-[250] ease-in group-data-[state=open]:-rotate-180 dark:text-zinc-400"
                            aria-hidden
                          />
                        </span>
                      </>
      ...
      <div
        as="nav"
        className="fixed bottom-4 left-0 right-0 z-10 mx-auto flex h-max w-max items-center justify-center rounded-full border border-zinc-200 bg-white/40 backdrop-blur-2xl p-1 shadow-lg duration-500 dark:border-zinc-800 dark:bg-zinc-900/40 dark:shadow-none md:hidden"
      >
      ...
      <NavigationMenu.Link asChild>
        <Link
          className={classNames(
            "block h-full w-full select-none rounded-md p-2 leading-none no-underline outline-none transition-colors hover:bg-zinc-200 focus:shadow-[0_0_0_2px] focus:shadow-zinc-300 dark:hover:bg-zinc-800",
            className
          )}
          {...props}
          ref={forwardedRef}
        >
          <div className="mb-2 font-sans font-bold leading-[1.2] text-zinc-800 dark:text-zinc-200">
            {title}
          </div>
